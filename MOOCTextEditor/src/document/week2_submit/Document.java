...
public abstract class Document {
	...
	protected int countSyllables(String word)
	{
	    ...
	    while (index < word.length()) {
	    	if (!set.contains(word.charAt(index)) || (index > 0 && set.contains(word.charAt(index - 1)))) {
	    		index++;
	    		continue;
	    	} else if (index == word.length() - 1 && (word.charAt(index) == 'e' || word.charAt(index) == 'E')) {
	    		break;
	    	} else {
	    		index++;
	    		count++;
	...
	public double getFleschScore()
	{
	    ...
	    return 206.835 - 1.015 * this.getNumWords() / this.getNumSentences() - 84.6 * getNumSyllables() / this.getNumWords();
	}
