...
public class AutoCompleteDictionaryTrie implements  Dictionary, AutoComplete {
	...
	public boolean addWord(String word)
	{
		...
		for (int i = 0; i < word.length(); i++) {
			TrieNode nextNode = curNode.insert(word.charAt(i));
			if (nextNode == null) {
				nextNode = curNode.getChild(word.charAt(i));
			} else {
				newNodeCreated = true;
			}
			curNode = nextNode;
		}
		if (newNodeCreated || !curNode.endsWord()) {
			newNodeCreated = true;
			size++;
		}
	...
	public boolean isWord(String s) 
	{
		...
		return lastNode != null && lastNode.endsWord();
	}
     ...
     */@Override
     public List<String> predictCompletions(String prefix, int numCompletions) 
     {
    	 ...
    	 List<String> predictWords = new LinkedList<>();
    	 if (lastNode == null) {
    		 return predictWords;
    	 }
         ...
         while (!queue.isEmpty() && numCompletions > 0) {
        	 ...
        	 for (Character c: nextChars) {
        		 queue.offer(curNode.getChild(c));
        	 }
         }
         return predictWords;
     }
